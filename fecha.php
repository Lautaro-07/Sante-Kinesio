...
if (isset($_GET['fecha'])) {
    ...
    if ($_SESSION['servicio'] == 'Terapia Manual - RPG') {
        if (!isset($disponibilidadProfesionales[$profesional]['Terapia Manual - RPG'][$diaSemana])) {
        ...
        // Bloquear permanentemente los días martes y jueves para los servicios "kinesiologia" y "drenaje linfatico" con Lucia
        if (($diaSemana == 'Tuesday' || $diaSemana == 'Thursday') && in_array($_SESSION['servicio'], ['kinesiologia', 'drenaje linfatico'])) {
            $horasDisponibles = []; // No hay horarios disponibles
        } else {
    ...
    if ($_SESSION['servicio'] == 'Rehabilitación') {
        $stmt = $mysqli->prepare("SELECT TIME_FORMAT(hora, '%H:%i') as hora, COUNT(*) as count FROM turnos WHERE profesional = ? AND fecha = ? AND servicio = 'Rehabilitación' GROUP BY hora HAVING count >= 1");
    } else {
