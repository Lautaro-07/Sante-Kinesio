...
if (isset($_GET['fecha'])) {
    ...
    if ($_SESSION['servicio'] == 'Rehabilitación') {
        $stmt = $mysqli->prepare("SELECT TIME_FORMAT(hora, '%H:%i') as hora, COUNT(*) as count FROM turnos WHERE profesional = ? AND fecha = ? AND servicio = 'Rehabilitación' GROUP BY hora HAVING count > 0");
    } else {
